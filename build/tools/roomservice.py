...
if not depsonly:
    print("Device %s not found. Attempting to retrieve device repositories from GitHub (http://github.com/Project-Mist-OS/)." % device)
...
if not depsonly:
    githubreq = urllib.request.Request("https://api.github.com/orgs/Project-Mist-OS/repos?per_page=100")
    add_auth(githubreq)
...
def add_to_manifest(repositories):
    ...
    for repository in repositories:
        ...
        project = ElementTree.Element("project", attrib = {
            "path": repo_target,
            "remote": "github",
            "name": "MistOS-devices/%s" % repo_name,
...
def get_default_or_fallback_revision(repo_name):
    ...
    githubreq = urllib.request.Request("https://api.github.com/repos/Project-Mist-OS/" + repo_name + "/branches")
    add_auth(githubreq)
    result = json.loads(urllib.request.urlopen(githubreq).read().decode())
